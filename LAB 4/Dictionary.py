...
if __name__ == '__main__':
    ...
    myList.pop(0)
    print(myList)
    myList.append("is the first inventor of")
    print(myList)
    ...
    print("Please enter the number of words")
    user_number = int(input())

    if 3 <= user_number <= 7:
        password = ""
        for x in range(user_number):
            password = password + words[random.randint(0, 1000)]
        print(password)
    else:
        print("Invalid number.")

    # Creating two sets for question 4
    s1 = set()
    s2 = set()
    for i in range(5):
        print("Please enter the number of words")
        ...
        if 3 <= user_number <= 7:
            password = ""
            for x in range(user_number):
                password = password + words[random.randint(0, 999)]
            print(password)
            if user_number <= 5:
                s1.add(password)
            else:
    ...
    sqr = {1: 1, 2: 4, 3: 9, 4: 16, 5: 25, 6: 36, 7: 49, 8: 64, 9: 81,
    ...
    for x in merged_dictionary:
        print(str(x)+": "+str(merged_dictionary.get(x)))
    merged_dictionary.pop('Nat')
    print(merged_dictionary)

    dicKeys = list(merged_dictionary.keys())
    dicKeys.sort()
    print(dicKeys)
    dicValues = list(merged_dictionary.values())
    dicValues.sort()
    print(dicValues)
    print("Name / Age")
    for i in range(5):
        print(dicKeys[i]+" "+str(dicValues[i]))
    sorted_merged_dictionary = list(merged_dictionary.values())
    sorted_merged_dictionary.sort()
    print()
    print("Max: "+str(sorted_merged_dictionary[4]))
    print("Min: "+str(sorted_merged_dictionary[0]))
