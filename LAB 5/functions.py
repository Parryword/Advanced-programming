...
def passGen(number):
    ...
    for x in range(int(number)):
        password += words[random.randint(0, 1000)]
    return password


def rep_with_upper(string):
    letter = str(string[random.randint(0, len(string) - 1)])
    print('Chosen letter: ' + letter)
    newstring = "";
    for x in str(string):
        if letter == x:
            newstring += x.upper()
...
def search_letter(sletter, string):
    for index, letter in enumerate(string):
        if string[index] == sletter:
            return True
...
if __name__ == '__main__':
    ...
    while True:
        ...
        user_number = input()
        if 3 <= int(user_number) <= 7:
            password = str(passGen(user_number))
            print('Initial password: ' + password)
            password = rep_with_upper(password)
            print('Upper case password: ' + password)
            password = swap_letters(password)
            print('Swapped password: ' + password)
            print('Found: ' + str(search_letter('x', password)))
            break
        else:
            print('Invalid value')
